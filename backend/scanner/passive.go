...
func (pm *PassiveMonitor) Start() {
	...
	if err != nil {
		slog.Warn("Passive monitor unavailable", "interface", pm.ifaceName, "error", err.Error())
		slog.Info("Note: gopacket requires root/capabilities and libpcap. Skipping passive monitor.")
		return
	}
	defer handle.Close()
	...
	if err != nil {
		slog.Error("Failed to set BPF filter", "error", err.Error())
		return
	}

	slog.Info("Passive monitor started", "interface", pm.ifaceName)
	packetSource := gopacket.NewPacketSource(handle, handle.LinkType())

	for {
		select {
		case <-pm.quit:
			slog.Info("Passive monitor stopped")
			return
...
func (pm *PassiveMonitor) handleDiscoveredDevice(ip, mac, source string) {
	...
	if err == sql.ErrNoRows {
		...
		if err == nil {
			slog.Info("New device discovered passively", "ip", ip, "mac", mac, "source", source)
